...
export default function AddItemPage() {
    ...
    // undefined = loading fixed by Vishnu
    // null = not logged in
    // number = logged in
    ...
    // Fetch Sub Categories new fixed by Vishnu
    useEffect(() => {
        if (!userId) return;
        ...
        (async () => {
            try {
                ...
                if (subs.length > 0){
                    ...
                    initializeSubCategory(subs[0].id, null);
                }
    ...
    // Runs when activeSubCategory changes
    useEffect(() => {
        if (activeSubCategory) {
            initializeSubCategory(activeSubCategory.id, null);

            fetchBrands(activeSubCategory.id);
            setModels([]);
        }
    }, [activeSubCategory]); // Depend on activeSubCategory
    ...
    const deleteItemFromDB = async (itemId) => {
        setAlert({ type: "info", message: `Deleting item...` });
    ...
    return (
        <>
            <head>
                <title>Add New Item | FindOnRent</title>
                <meta name="description" content="Add your vehicles or rental items easily on FindOnRent. Fill details, upload images, set pricing, and publish your listing."/>
                <meta property="og:title" content="Add New Item | FindOnRent" />
                <meta property="og:description" content="Add your vehicles or rental items easily on FindOnRent. Fill details, upload images, set pricing, and publish your listing."/>
                {/* <meta
                    property="og:image"
                    content="/images/assets/default-listing-banner.jpg"
                /> */}
                <meta property="og:type" content="website" />
                <meta property="og:url" content="https://findonrent.com/add-item"/>
                <link rel="canonical" href="https://findonrent.com/add-item" />
            </head>
                                    ...
                                    {(category) => (
                                        <div className="card border-0">
                                            <div className={`card-body ${styles.roh_addItems_formWrapper}`}>
                                                ...
                                                {items.filter((i) => i.sub_cat_id === category.id).length > 0 && (
                                                    <div className={styles.roh_addedItem_wrapper}>
                                                        ...
                                                        <div className={styles.roh_summerycard_grid}>
                                                            {items.filter((i) => i.sub_cat_id === category.id).map((item) => (
                                                ...
                                                {/* VEHICLE FORM */}
                                                <VehicleForm category={category} brands={brands} models={models} currentItem={currentItem} activeSubCategory={activeSubCategory} initializeSubCategory={initializeSubCategory} removeSubCategoryIfEmpty={removeSubCategoryIfEmpty} updateItemField={updateItemField} fetchModels={fetchModels} onSubmit={handleAddItem} />
                                            </div>
                                        </div>
                                    )}
                                </CategoryTabs>
            ...
            {showSuccessModal && (
                <div className={styles.roh_modal_overlay}>
                    <div className={styles.roh_modal_box}>
                        <h2>🎉 Success!</h2>
                        <p>Your items have been successfully submitted.</p>

                        <div className={styles.roh_modal_actions}>
                            ...
                            <button onClick={() => router.push(`/dashboard?view=items`)}>
                                Go to Dashboard
                            </button>
                        </div>
                    </div>
