"use client";
import "../../../globals.css"
import styles from "./recreational.module.css";
...
export default function Vehiclescars() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch(`${ROH_PUBLIC_API_BASE_URL}/getvehiclesbrands`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ cat_id: 6 }),
        });
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const url = new URL(`${ROH_PUBLIC_API_BASE_URL}/getallrecreationalvehicles`);
        url.searchParams.set("page", String(initialPage));
        url.searchParams.set("limit", String(limit));
        url.searchParams.set("cat_id", "6");
                ...
                <div className={styles.custom_searchbar}>
                  ...
                  <form className="w-100" method="GET" action="">
                    ...
                    <div className={`row ${styles.roh_filtter_top_bar}`}>
                      ...
                      {/* Tag */}
                      <div className={`${styles.roh_fiiters} ${styles.roh_boder_bottumMob}`}>
                        <div className="form-group w-100">
                          <label htmlFor="tag" className={`${styles.cat_block_inner}`}>
                            Tag
                          </label>
                          <div className={styles.category_in_wrap}>
                            <select
                          ...
                          <div className={`${styles.search_block_inner} rounded-pill`}>
                            ...
                            <div className={`${styles.roh_searchinput_btn_wrapper}`}>
                              <div className={`${styles.search_wraperInput}`}>
                                ...
                                <Image src="/search-icon.svg" width={25} height={25} alt="Search Input Icon" />
                                <input
                                  id="whatoftype"
                              ...
                              <div className={styles.rent_search_btn}>
                                ...
                                <a href="/services/vehicles/cars" className={`${styles.roh_search_clearBtn}`} type="clear">
                                  Clear
                                </a>
                              </div>
                            </div>
              ...
              <div className="col-12">
                <div className="bottom_title">
                    <ul className={styles.list_bottom}>
                      <li><span style={{color:"#ff3600"}}><LuBadgePercent size={20}/></span> Zero Commission</li>
                      <li><span style={{color:"#ff3600"}}><LuHandshake size={20}/></span> Direct Vendor Bookings</li>
                      <li><span style={{color:"#ff3600"}}><LuShieldCheck size={20}/></span> Hassle-free Rentals</li>
                    </ul>
                  </div>
              </div>
            </div>
                  ...
                  products.map((p) => (
                      <div className={`card ${styles.fleetscard} h-100`}>
                        <img
                          src={
                            p?.media_gallery?.[0]
                              ...
                              : `${WEB_BASE_URL}/uploads/media/host/items/placeholder.png`  // Updated this line
                          }
                        ...
                        >
                          ...
                          <div className={`${styles.roh_feets_list} d-flex justify-content-between text-secondary mb-3`}>
                            <div className="d-flex align-items-center gap-1 feets_data_list">
                              <Image
                          ...
                          <div className="d-flex justify-content-between align-items-center mt-auto pt-2" style={{borderTop: "1px dashed #E5E7EB"}}>
                            ...
                            <a
                              ...
                              onClick={() => {  trackItemView(p.id);  setSelectedId(p.id); }}
                            >
                ...
                {Array.from({ length: totalPages }).map((_, i) => {
                  ...
                  return (
                    <a
                      key={pnum}
                      href={url}
                      className={`${styles.rohproducts_btn_page} ${page === pnum
