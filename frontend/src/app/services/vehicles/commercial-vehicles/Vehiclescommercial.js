...
export default function Vehiclescommercial() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const url = new URL(`${ROH_PUBLIC_API_BASE_URL}/getallcommercialvehicles`);
        url.searchParams.set("page", String(initialPage));
        url.searchParams.set("limit", String(limit));
        url.searchParams.set("cat_id", "4");
