...
export default function Vehiclesbikes() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true);
      try {
        const url = new URL(`${ROH_PUBLIC_API_BASE_URL}/getallvehiclesbikes`);
        url.searchParams.set("page", String(initialPage));
        url.searchParams.set("limit", String(limit));
        url.searchParams.set("cat_id", "3");
          ...
          <div className="container px-2 px-md-3 px-lg-3 position-relative mt-4 mt-lg-5">
            <div className="row d-flex justify-content-start g-3">
              ...
              <div className={styles.roh_vehicleslisting_grid}>
                {!loading &&
                  products.map((p) => (
                    <a className={`card ${styles.fleetscard} h-100`}
                      ...
                      <img
                        // src={ p?.media_gallery?.[0] ? `${WEB_BASE_URL}${p.media_gallery[0].file_path}${p.media_gallery[0].file_name}` : `${WEB_BASE_URL}/uploads/media/host/items/placeholder.png`}
