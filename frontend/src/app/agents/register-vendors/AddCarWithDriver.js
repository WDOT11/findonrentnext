...
export default function CarWithDriverForm({ subCategory, data, setData }) {
  const [brands, setBrands] = useState([]);
  const [modelsByItem, setModelsByItem] = useState({});

  // Ensure items is always an array
  const rawData = data.itemsBySubCategory?.[subCategory.id];
  const items = Array.isArray(rawData) && rawData.length > 0 ? rawData : [{ images: [] }];
      ...
      {items.map((item, index) => (
        <div key={index} className="border p-4 rounded mb-4 bg-white shadow-sm position-relative">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h5 className="m-0 text-dark">Cars With Driver #{index + 1}</h5>
            {items.length > 1 && (
              <button
                type="button"
          ...
          <div className="row g-3 mb-3">
            <div className="col-md-6">
              <label className="form-label fw-bold">Car Name <span className="text-danger">*</span></label>
              <input
                type="text"
                className="form-control"
                placeholder="e.g. Luxury Sedan with Driver"
                value={item.title || ""}
          ...
          <div className="row g-3 mb-3">
            ...
            <div className="col-md-6">
              <label className="form-label fw-bold small">Rate Per KM (Optional)</label>
              <div className="input-group">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    updateField(index, "ratePerKM", val);
                  }}
                />
              </div>
            </div>
          ...
          <div className="mb-3 mt-4">
            ...
            <div className="d-flex flex-wrap gap-3 align-items-start">
              ...
              <div>
                ...
                />
                <label
                  htmlFor={`car-driver-image-${index}`}
