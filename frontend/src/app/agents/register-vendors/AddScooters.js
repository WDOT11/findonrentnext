...
export default function ScooterForm({ subCategory, data, setData }) {
  const [brands, setBrands] = useState([]);
  const [modelsByItem, setModelsByItem] = useState({});
  const defaultScooter = { engine: "Petrol", images: [] };

  // Ensure items is always an array
  const rawData = data.itemsBySubCategory?.[subCategory.id];
  // const items = Array.isArray(rawData) ? rawData : [{}];
  const items = Array.isArray(rawData) && rawData.length > 0 ? rawData : [defaultScooter];
  ...
  const updateField = (index, field, value) => {
    setData((prev) => {
      ...
      const currentSubCatItems = prev.itemsBySubCategory?.[subCategory.id] || [defaultScooter];
  ...
  const addItem = () => {
    setData((prev) => ({
      ...
      itemsBySubCategory: {
        ...prev.itemsBySubCategory,
        [subCategory.id]: [...items, defaultScooter],
      },
    }));
  };
      ...
      {items.map((item, index) => (
        <div key={index} className="border p-4 rounded mb-4 bg-white shadow-sm position-relative">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h5 className="m-0 text-dark">Scooter #{index + 1}</h5>
            {items.length > 1 && (
              <button
                type="button"
          ...
          <div className="row g-3 mb-3">
            <div className="col-md-6">
              <label className="form-label fw-bold">Scooter Name <span className="text-danger">*</span></label>
              <input
                type="text"
          ...
          <div className="mb-3 mt-4">
            <label className="form-label fw-bold">Scooter Images</label>
