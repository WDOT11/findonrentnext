...
export default function VehicleList({initialProducts, total, categories, categorySlug, categorySinName, locSlug, brands, cate_id, modelName, modelSlug, modelLabel, categoryName}){
  ...
  useEffect(() => {
    ...
    const normalized = Array.isArray(brands) ? brands : brands?.brands || [];
  ...
  const renderCategoryLocationBreadcrumb = (categorySlug, locationSlug, categoryFullName) => {
    ...
    const readableCategory = categorySinName
      .replace(/-/g, " ")
    ...
    return (
      <nav aria-label="Breadcrumb" className={styles.breadcrumbWrap}>
        <ol className={styles.breadcrumbList}>
          ...
          <li>
            <a href={`/${categorySlug}`}>{categoryFullName}</a>
          </li>

          <li className={styles.separator}>›</li>
          <li>
            <a href={`/${categorySlug}/${modelSlug}`}>{modelLabel || modelName}</a>
          </li>
          ...
          <div className="row justify-content-center">
            <div className="col-12">
              <div className={styles.main_heading}>
                {/* <h1>Cars</h1> */}
                <h1 className="mb-0 mb-sm-3">{`${modelLabel || modelName} on rent in ${locSlug}`}</h1>
                {/* Breadcrumb here */}
      ...
      <div className={styles.fleetswrap_inner}>
        <div className={styles.fleets_wrap_main}>
          ...
          <h3 id="fleet-heading" className="roh_section_title_h3 text-center">
            ...
            <br />
            <span>
              {`extensive fleet of ${modelLabel || modelSlug} ${categoryFullName} in ${locSlug}`}
            </span>
          </h3>
          ...
          <div className="container px-2 px-md-3 px-lg-3 position-relative mt-4 mt-lg-5">
            <div className="row d-flex justify-content-start g-3">
              ...
              <div className={styles[gridClass]} aria-label="Fleets list">
                {products.map((p) => (
                        ...
                        <div className={`${styles.roh_feets_list} d-flex justify-content-between text-secondary mb-2 mb-lg-3`}>
                          <div className={styles.roh_feets_data_list}>
                            <div><LuStore size={18} style={{color:"#9CA3AF"}} aria-hidden="true"/></div>
                             <a href={`/rental-service-provider/${p?.business_slug}`} target="_blank" rel="noopener" className={styles.roh_vendor_link}  aria-label={`View rental provider ${p?.business_name}`}
                              onClick={(e) => {
                                e.stopPropagation();
                              }}
                             >
            ...
            {totalPages > 1 && (
              <div
                className={`${styles.rohproducts_pagination} d-flex justify-content-center mt-5 gap-2 flex-wrap`}
              >
                {/* Prev */}
                {page > 1 && (
                  <a
