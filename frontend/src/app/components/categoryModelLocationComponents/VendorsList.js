...
export default function VendorsList({ businessIds, categorySinName }) {

  const [rentalProviders, setRentalProviders] = useState([]);
  const [limit, setLimit] = useState(8);
  const business_ids = businessIds;
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(
          `${ROH_PUBLIC_API_BASE_URL}/getVendorsByBusinessIds`,
          {
            method: "POST",
            headers: {
  ...
  const getServicesText = (categories = []) => {
    ...
    return categories.map(cat => cat.sub_category_name).filter(Boolean).join(", ");
  };
      ...
      {rentalProviders && rentalProviders.length > 0 && (
        <div className={`${styles.roh_provider_section_wrapper}`}  aria-label={`Top rated ${categorySinName} rental providers in ${displayCityName}`}>
          <div className={styles.roh_section_header}>
            <h2 className="mt-0">{`Top rated ${categorySinName} rental providers in ${displayCityName}`}</h2>
              ...
              <div key={vendor.user_id} className={styles.roh_provider_card} aria-label={`Rental provider ${vendor.business_name} in ${displayCityName}`}>
                <div className={styles.roh_provider_card_top}>
                  <div className={styles.roh_provider_card_info}>
                    <div className="d-flex align-items-start justify-content-between">
                      <a href={`/rental-service-provider/${vendor.business_slug}`} target="_blank" rel="noopener" aria-label={`View details of ${vendor.business_name}`}>
                ...
                <div className={styles.roh_provider_card_body}>
                  <div className={styles.roh_meta_item}>
                    ...
                    <span className={styles.roh_meta_value}>{displayCityName}, {displayStateName}</span>
                  </div>
                  <div className={styles.roh_meta_item}>
                ...
                <div className={styles.roh_provider_card_footer}>
                  <a href={`/rental-service-provider/${vendor.business_slug}`} className={styles.roh_btn_card_secondary} target="_blank" rel="noopener" aria-label={`View rental provider ${vendor.business_name}`}>
                    View Provider
                  </a>
                </div>
              </div>
          ...
          <div className={styles.roh_view_all_container}>
            <a href={`/rental-service-providers?city=${citySlug}`} className={styles.roh_btn_view_all} aria-label={`View all rental providers in ${displayCityName}`}>
