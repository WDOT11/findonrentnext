...
export default function VendorsList({ cat_id, user_city, categorySinName }) {

  const [rentalProviders, setRentalProviders] = useState([]);
  const [limit, setLimit] = useState(8);
  const locationSlug = user_city;
            ...
            {rentalProviders.map((vendor) => (
              <div key={vendor.user_id} className={styles.roh_provider_card}>
                <div className={styles.roh_provider_card_top}>
                  <div className={styles.roh_provider_card_info}>
                    <div className="d-flex align-items-start justify-content-between">
                      <a href={`${WEB_BASE_DOMAIN_URL}/rental-service-provider/${vendor.business_slug}`} target="_blank" rel="noopener" aria-label={`View rental provider ${vendor.business_name} in ${displayCityName}`}>
                        <h2>{vendor.business_name}</h2>
                      </a>
                      {vendor.is_verified == 1 && (
                        <span className={styles.roh_provider_badge}>
                          <img src="/verified.svg" alt="Verified" aria-hidden="true" />
                        </span>
                      )}
                    </div>
                ...
                <div className={styles.roh_provider_card_footer}>
                  <a href={ WEB_BASE_DOMAIN_URL + `/rental-service-provider/${vendor.business_slug}`} className={styles.roh_btn_card_secondary} target="_blank" rel="noopener" aria-label={`View details of rental provider ${vendor.business_name}`}>
                    View Provider
                  </a>
                </div>
              </div>
