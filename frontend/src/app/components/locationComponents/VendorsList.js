...
export default function VendorsList({ loc_slug }) {
    ...
    (async () => {
      try {
        const res = await fetch(
          `${ROH_PUBLIC_API_BASE_URL}/getVendorsByLocOrCat?limit=${limit}&locationSlug=${locationSlug}`
        );
  ...
  return (
    <>
      {rentalProviders && rentalProviders.length > 0 && (
        <div className={`${styles.roh_provider_section_wrapper}`} aria-label={`Top rated rental providers in ${displayCityName}`}>
          <div className={styles.roh_section_header}>
            <h2 className="mt-0">
              {`Top Rated Rental Providers in ${displayCityName}`}
            </h2>
            ...
            {rentalProviders.map((vendor) => (
              <div key={vendor.user_id} className={styles.roh_provider_card}>
                ...
                <div className={styles.roh_provider_card_footer}>
                  <a href={`${WEB_BASE_DOMAIN_URL}/rental-service-provider/${vendor.business_slug}`} className={styles.roh_btn_card_secondary} target="_blank" rel="noopener" aria-label={`View ${vendor.business_name} rental provider profile`}>
                    View Provider
                  </a>
                </div>
              </div>
